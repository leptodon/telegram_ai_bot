...
class Config:
    ...
    def from_env(cls):
        ...
        if os.path.exists(env_file):
            ...
            print("🔍 Проверьте, что файл .env находится в корне проекта")

        # Отладочная печать для проверки загрузки .env
        ...
        env_vars = {
            ...
            'MESSAGE_PROBABILITY': os.getenv('MESSAGE_PROBABILITY'),
        ...
        for key, value in env_vars.items():
            if value is not None:
                ...
                print(f"❌ {key}: НЕ НАЙДЕНО (будет использовано значение по умолчанию)")
